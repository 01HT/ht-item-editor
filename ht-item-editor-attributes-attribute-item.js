...
class HTItemEditorAttributesAttributeItem extends LitElement {
  ...
  render() {
    ...
    return html`
      ...
      <div id="container">
          <div id="actions">
              <paper-icon-button icon="ht-item-editor-categories-item:add-box" @click="${this.open()}" ?hidden="${opened}" ?hide="${
      this.showBlockHidden
    }"></paper-icon-button>
              <paper-icon-button icon="ht-item-editor-categories-item:indeterminate-check-box" @click="${this.close()}" ?hidden="${!opened}" ?hide="${this.showBlockHidden()}"></paper-icon-button>
          <div id="title" class="${
            this.isRoot() ? "root" : ""
